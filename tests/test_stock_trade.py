...
class TestStockTrade:
    """虚拟货币交易功能测试"""
    def test_hk_stock_trade_buy(self, main_page):
        """提交买入限价单，并撤单"""
        logging.info("开始执行买入限价单测试")
        try:
            main_page.swich_account("STOCK")
            main_page.click_element(("xpath", "//*[contains(@content-desc,'股票代码')]"))#点击顶部搜索框
            main_page.input_text(("xpath", "//android.widget.EditText"), HK_STOCK_CODE)#输入搜索内容
            time.sleep(0.5)
            main_page.wait_and_click_element(("xpath", f"//*[contains(@content-desc,'{HK_STOCK_CODE}')]"))#等待搜索结果出现后，并点击
            ...
            #计算买入价格
            price  = str(new_price*0.8)
            logging.info(f"计算买入价格: {price}")

            #展开价格输入的数字键盘，清除原价格，输入新价格
            main_page.click_element(("xpath", "//android.widget.EditText[@index='9']"))
            ...
            #展示数量输入的数字键盘，并输入数量
            main_page.click_element(("xpath", "//*[contains(@content-desc,'价格最小单位')]/following-sibling::*[1]"))
            time.sleep(1)
            main_page.wait_for_element_visible(("xpath", "//*[@content-desc='完成']"))
            for key in "4000":
            ...
            #提交订单
            main_page.click_element(("xpath", "//android.widget.Button[@content-desc='买入']"))
            ...
            logging.error(f"买入单测试执行失败: {str(e)}", exc_info=True)
            raise
